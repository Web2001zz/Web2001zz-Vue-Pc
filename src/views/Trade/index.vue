...
export default {
  ...
  methods: {
    async submit() {
      ...
      const orderId = await reqSubmitOrder({
        tradeNo, //订单编号
        consignee, //收件人姓名
        detailArrayList: phoneNum, //收件人电话
        deliveryAddress: userAddress, //选择的配送的地址
        paymentWay: "ONLINE", //是否在线支付
        orderComment: this.orderComment, //订单备注
        orderDetailList: detailArrayList, //提交的所有商品列表
      });

      //跳转到支付页面并将订单号传给query参数
      this.$router.push({
        path: "/pay",
        query: {
          orderId,
  ...
  async mounted() {
    ...
    this.trade = trade;
    //用户地址信息
    this.userAddressList = trade.userAddressList;
    //获取默认地址的id
